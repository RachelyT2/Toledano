...
async function safeSendMail({ from, to, subject, text, html, replyTo, attachments }) {
...
async function sendVerificationEmail(email, name, token, baseUrl) {
  try {
    const link = `${baseUrl}/verify?token=${token}`;
    const subject = 'אימות כתובת אימייל - משפחה';
    const text = `שלום ${name || ''},\n\nנא לאשר את כתובת הדוא'ל:\n${link}`;
    const html = `<p>שלום ${escapeHtml(name || '')},</p><p><a href="${link}">${link}</a></p>`;
    await safeSendMail({ from: FROM_ADDRESS, to: email, subject, text, html });
  } catch (e) {
    console.error('sendVerificationEmail failed', e.message);
  }
}
  ...
  try {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;">
        <h2>שלום ${escapeHtml(name || '')},</h2>
        <p>סיסמה זמנית: <code>${escapeHtml(tempPassword)}</code></p>
        <p><a href="${link}">אשר כתובת מייל</a></p>
      </div>
    `;
    await safeSendMail({ from: FROM_ADDRESS, to: email, subject, text, html });
  } catch (e) {
    console.error('sendNewUserWelcomeEmail failed', e.message);
  }
}

// getTransport נשמר לתאימות — לא בשימוש עם Resend
async function getTransport() {
  return null;
}
